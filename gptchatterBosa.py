...
ser = serial.Serial('COM6', 9600)  # Vervang 'COM1' door de juiste poort en 9600 door de juiste baudrate
    ...
    while True:
        ...
        if data:
            if button_code == 8:  # Previous Slide button code
                ...
                #     bs_pressed = True
                # if bs_pressed:
                #     press_duration = time.time() - bs_pressed_time
                #     if press_duration > 2:
                #         print("Context Reset")
                #         reset_conversation = True
                #         bs_pressed_time = time.time()
...
while True:
    ...
    if (firstCall != True) :
        ...
        if (chatViaMic == False) :
            ...
            print("Druk kort op de knop naar links van de controller")
            while (True) :
                data = device.read(endpoint.bEndpointAddress, endpoint.wMaxPacketSize, timeout=50000000)
                button_code = data[0]
                if (button_code == 8):
